...
make_coverage_plot = function(region, distance = 5000, seurat_object = g4, result_folder = "results/cicero/") {
  chr = strsplit(region, "-")[[1]][1]
  start = as.numeric(strsplit(region, "-")[[1]][2]) - distance
  end = as.numeric(strsplit(region, "-")[[1]][3]) + distance
  p = CoveragePlot(seurat_object, region = glue("{chr}-{start}-{end}"),
    annotation = TRUE, show.bulk = TRUE, group.by = "AST_status",
    ranges.title = region, ranges = li_enh, height.tracks = 20)
  annot = AnnotationPlot(object = g4, region = region)
  ggsave(glue("{result_folder}{region}_cicero_covplots.pdf"), plot = p, width = 12, height = 8, device = "pdf")
  ggsave(glue("{result_folder}{region}_annotplot.pdf"), plot = annot, width = 12, height = 8, device = "pdf")
  return(print(p))
}

make_coverage_plot_all = function(region, distance = 5000, seurat_object, result_folder) {
  ...
  plots = ggarrange(plotlist = list(nonAST_example, AST_example), ncol = 2, nrow = 1)
  ggsave(glue("{result_folder}{region}_cicero_arranged_covplots.pdf"), plot = plots, width = 12, height = 8, device = "pdf")
}

# data
mm10.chromsize = fread("../../data/mm10.chrom.sizes.txt")
g4 = readRDS(file = "../../data/GSE291468/GSM8836086_GFPpos_Seurat_object.Rds")
pred = fread("results/scBridge/scbridge_predictions.csv", header = TRUE)
...
##
load("results/cicero/cicero_GFPsorted-predAST.Rds")
load(
  "results/cicero/cicero_GFPsorted_coG4networks-predAST.Rds"
)
AST = conns
AST_ccan = CCAN_assigns
rm(conns)
load("results/cicero/cicero_GFPsorted-pred_nonAST.Rds")
load(
  "results/cicero/cicero_GFPsorted_coG4networks-pred_nonAST.Rds"
)
nonAST = conns
