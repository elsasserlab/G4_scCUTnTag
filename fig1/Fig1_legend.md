...
**Panel E — PCA of cluster-specific peaks.** Summit-centered regions (±250 bp from narrowPeak summit) unique to cluster0 or cluster1 (no overlap between clusters; 10,408 cl0_only + 13,586 cl1_only = 23,994 total). PCA performed on RPGC-normalized signal (computed with `wigglescout::bw_loci()`) at these regions across six bigWig tracks: cluster0, cluster1, GSM8836082, GSM8836083, GSM8836084, GSM8836085. Plotted as binary PCA (samples present/absent) and continuous PCA. Tools: wigglescout, GenomicRanges, ggplot2 `prcomp()`. Outputs: `panel_E_binary.pdf`, `panel_E_pca.pdf`, `panel_E_marker_regions.mm10.bed`.
...
**Panel S1 — Spearman and Pearson correlation heatmaps.** Correlations among four bigWig tracks (cluster0, cluster1, GSM8836082, GSM8836084) at (a) cluster-specific peaks (cl0_only + cl1_only; 23,994 total), and (b) |log2FC| > 2 filtered peaks including shared regions (10,825 peaks). Signal extracted with `wigglescout::bw_loci()`, correlations computed with `cor(method="spearman")` and `cor(method="pearson")`, heatmaps rendered with ComplexHeatmap `Heatmap()`. Outputs: `panel_S1.pdf` (Spearman + Pearson, 23,994 peaks), `panel_S1_lfc2.pdf` (Spearman + Pearson, |lfc|>2 filtered), `panel_S1_spearman_correlation.csv`, `panel_S1_pearson_correlation.csv`, `panel_S1_lfc2_spearman_correlation.csv`, `panel_S1_lfc2_pearson_correlation.csv`, `panel_S1_marker_regions.mm10.bed`.
