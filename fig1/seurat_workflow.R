...
# e.g. "results/seurat_mESC_MEF/" or "results/seurat_unsorted_mousebrain_res0.8/"
# add path to CellRanger output folder
# e.g. "../../data/GSE291468/GSM8836088_mESC_MEF/CellRanger/"
# cluster resolution (0.1 or 0.8 were used)
if (all(sapply(args, is.null))) {
  print("Script is running without sbatch.")
  res = 0.1
  workdir = "results/seurat_unsorted_res0.1/"
  cell_ranger = "../../data/GSE291468/GSM8836087_unsorted_mouse_brain/CellRanger/"
  sample = "unsorted"
} else {
...
# Map sample names to GEO accession filenames
geo_names <- c(
  sorted   = "GSM8836086_scG4CnT_MouseBrain_GFPpos_Seurat_object.Rds",
  unsorted = "GSM8836087_scG4CnT_MouseBrain_unsorted_Seurat_object.Rds",
  mESC_MEF = "GSM8836088_scG4CnT_mixture_mES_3T3_Seurat_object.Rds"
)
out_rds <- if (sample %in% names(geo_names)) {
  file.path(workdir, "outputs", geo_names[[sample]])
} else {
  file.path(workdir, "outputs", "Seurat_object.Rds")
}
