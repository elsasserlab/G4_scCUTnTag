...
#
# Requires:
#   - fig2/results/integration/outputs/
...
if ("B" %in% run_panels) {
  ...
  emb$reliability <- as.numeric(joint$pred_max_score)
  ...
  p_rel <- ggplot(emb[emb$modality == "G4 scCUT&Tag", ], aes(UMAP1, UMAP2, color = reliability)) +
    ...
    labs(title = "Prediction scores", color = NULL) + theme_classic()
  ...
  if ("9630013A20Rik" %in% top) {
   ...
   if (length(top) == 0) {
    message("Panel F: no marker genes available")
     ...
     # Add Mbp (strong G4 signal in oligodendrocytes)
     if (!"Mbp" %in% top && "Mbp" %in% rownames(g4_data)) {
       top <- c(top, "Mbp")
     }
...
save_fig(wrap_plots(rna_plots, ncol = 3), "panel_F_RNA_featureplots", 15, 18)
     ...
     message("  Saved ", length(top), " marker gene feature plots (RNA + G4, including Mbp)")
     ...
     save_fig(wrap_plots(combined_plots, ncol = 4), "panel_F_combined_RNA_G4", 20, 13.5)
